...
__kernel void akima_calc_derivatives(__global const int *mask_weights, __global const int *mask_index_transform, __global real_t *m, __global const real_t *values, const int valueCount)
{
...
__kernel void akima_fill_boundary_derivatives(__global float *m, __global const int *maskedCounts, const int valueCount)
{
    int mask = get_global_id(0);
...
__kernel void akima_calc_parameters(__global const int *mask_weights, __global const int *mask_index_transform, __global real_t *m, __global const real_t *values, const int valueCount,
                                    __global real_t *a0, __global real_t *a1, __global real_t *a2, __global real_t *a3)
{
    ...
    else*/
    {
        ...
        const real_t nextA1 = (tmpWeight1 * m[LINEAR_2D_INDEX_M(mask, valueCount, myWork - 1 + 1)] + tmpWeight2 * m[LINEAR_2D_INDEX_M(mask, valueCount, myWork + 1)]) / wsum;
