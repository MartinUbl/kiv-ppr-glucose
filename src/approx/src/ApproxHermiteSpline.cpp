...
void ApproxHermiteSpline::CalculateParametersForMask(const uint32_t mask)
{
...
void ApproxHermiteSpline::CalculateParameters_threads()
{
...
void ApproxHermiteSpline::CalculateParameters_TBB()
{
    size_t maskedCount[APPROX_MASK_COUNT];
...
void ApproxHermiteSpline::CalculateParameters_OpenCL()
{
    cl_int ret;

    clProgramRecord* program = GetCLProgramRecord(apxmCubicHermiteSpline);
    if (!program)
        return;
    ...
    cl_kernel kernel = clCreateKernel(program->prog, "hermite_calc_parameters", &ret);
    ...
    else
    {
        ...
        for (int mask = 0; mask < APPROX_MASK_COUNT; mask++)
            for (int j = 0; j < maskedCount[mask]; j++)
                for (int k = 0; k < 4; k++)
                    xCoefs[mask][j][k] = (floattype)(tmp[mask * valueCount * 4 + j * 4 + k]);
...
HRESULT IfaceCalling ApproxHermiteSpline::GetLevels(floattype desiredtime, floattype stepping, size_t count, floattype *levels, size_t *filled, size_t derivationorder)
{
...
HRESULT ApproxHermiteSpline::GetIndexFor(floattype time, size_t &index, size_t &origIndex, uint8_t mask)
{
